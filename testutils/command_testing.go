...
type TestCaseRootParseAndExecute struct {
	Name  string
	Args  []string
	Want  int
	Setup SetupFunc[TestCaseRootParseAndExecute]
}

func RunTestRoot_ParseAndExecute(
	t *testing.T,
	tests []TestCaseRootParseAndExecute,
	testdata *TestData,
	newCmd Factory,
	customAssertion CustomAssertion[TestCaseRootParseAndExecute],
) {
	for _, tt := range tests {
		t.Run(tt.Name, func(t *testing.T) {
			cmd, outWriter, errWriter := setup[TestCaseRootParseAndExecute](t, tt, newCmd, tt.Setup)

			root, ok := cmd.(*mycmd.Root)
			if !ok {
				t.Fatal("cmd is not Root")
			}

			actual := root.ParseAndExecute(tt.Args)
